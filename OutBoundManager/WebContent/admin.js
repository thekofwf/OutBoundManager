...
function doLogin() {
	var url = getHref("main/loginEx" + "?userName=" + _uieTextboxGetValue("userName") + "&agentPwd=" + _uieTextboxGetValue("agentPwd"));
...
function handleLogin(result, data, statusText, xhr) {
	...
	try {
		...
		_uieTextboxSetValue("userInfo", jObj.userInfo);
		document.loginForm.action = getHref("work/admin");
